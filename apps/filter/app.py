...
def _hk_segment_status() -> List[dict]:
    return get_hk_enrich_segment_status(load_snapshot())
...
def _run_hk_segment_enrich(segment_key: str, segment_label: str, segment_count: int, *, force_refresh: bool, safe_mode: bool) -> None:
    if int(segment_count or 0) <= 0:
        st.info(f"{segment_label} 当前没有可深补股票。")
        return
    status_rows = _hk_segment_status()
    status_map = {str(row.get("key")): row for row in status_rows}
    current_row = status_map.get(str(segment_key), {})
    persisted_count = int(current_row.get("persisted_count", 0) or 0)
    pending_count = max(0, int(segment_count or 0) - persisted_count)
    ...
    with st.spinner(f"正在深补 {segment_label} ..."):
        try:
            ...
            if bool(stats.get("fallback", False)):
                st.warning(_format_ops_fallback_warning("港股", stats, weekly=False))
            else:
                mode_label = "补缺" if bool(stats.get("only_missing_enrich", False)) else "全量重补"
                base_total = int(stats.get("segment_pending", 0) or 0) if bool(stats.get("only_missing_enrich", False)) else int(stats.get("segment_total", 0) or 0)
...
def _render_market_ops_tab(scope: str, *, key_prefix: str) -> None:
    ...
    if scope in {"A", "HK"}:
        ...
        governance = get_enrichment_governance_summary(scope)
...
def _render_ops_panel() -> None:
    ...
    if ops_view == "A股":
        ...
        governance = get_enrichment_governance_summary("ALL")
        ...
        c1.metric("总样本", total)
        c2.metric("已覆盖", covered)
        c3.metric("部分/稀疏", int(completeness.get("partial", 0) or 0) + int(completeness.get("sparse", 0) or 0))
        ...
        if action_col1.button("生成体检报告（Excel）", width="stretch", key="ops_generate_health_xlsx"):
            st.session_state["ops_health_xlsx"] = export_snapshot_health_excel(days=30, top_n=50)
            st.session_state["ops_health_xlsx_name"] = f"filter_health_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        if st.session_state.get("ops_health_xlsx"):
            action_col1.download_button(
                "下载体检报告（Excel）",
                data=st.session_state["ops_health_xlsx"],
                file_name=st.session_state.get("ops_health_xlsx_name", "filter_health_report.xlsx"),
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="ops_download_health_xlsx",
            )
        ...
        render_data_vault_panel(key_prefix="filter_ops_vault")
...
if st.session_state["show_ops_panel"]:
    ...
    st.caption("数据运维台已收起，需要维护数据时可在侧边栏打开。")
st.markdown("---")
